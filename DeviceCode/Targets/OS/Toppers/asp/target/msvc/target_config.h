...
enable_int(INT intno)
{
	// Windows 用の処理を入れること
	return(true);
}

#define t_enable_int(intno)		enable_int(intno)
#define i_enable_int(intno)		enable_int(intno)
...
clear_int(INT intno)
{
	...
	//! ハンドラ番号のオブジェクトを取得する.
	// p = (WIINTOBJ *)wi_FindObject(intno, TMAX_MAXINT, ObjList, &ercd);
	// if (p){
	// 	//! 割り込み要求をクリアする.
	// 	p->IntrCnt = 0;
	// }
...
#define t_clear_int(intno)		clear_int(intno)
#define i_clear_int(intno)		clear_int(intno)

/*
 *  割込み要求のチェック
 */
Inline bool_t
x_probe_int(INT intno)
{
	// sigset_t	sigmask;
	// 
	// sigpending(&sigmask);
...
x_define_inh(INHNO inhno, FP int_entry, PRI intpri)
{
	...
	// p = (WIINTOBJ *)wi_GetObject(inhno, ObjList);
	// if (p){
	// 	p->IntrAdr = int_entry;
	// 	p->Enabled = FALSE;
	// 	p->IntrCnt = 0;
	...
	// p = (WIINTOBJ *)wi_CreateObject(inhno, TMAX_MAXINT, sizeof(WIINTOBJ), obj, ObjList, &ercd);
	// if (!p){
	// 	return ercd;
	// }
	//! リストに割り込みハンドラを追加する.
