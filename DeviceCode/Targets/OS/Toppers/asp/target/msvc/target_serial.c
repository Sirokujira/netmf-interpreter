...
static BOOL CALLBACK ConsoleProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	case SERMSG_CREATE:
	{
						  ...
						  hConsole = NULL; // GetDlgItem(hDlg, IDC_CONSOLE);
	...
	case WM_SIZE:
	{
					...
					MoveWindow(NULL, 0, 0, client.right, client.bottom, TRUE);
					break;
	}
...
static BOOL SerialConsole_PutChar(SIOPCB * cb, INT chr, BOOL rasint)
{
	...
	console = GetDlgItem(cb->Handle, NULL);
	buffer[0] = (char)chr;
	buffer[1] = '\x0';
...
static void WinConsole_CreatePort(SIOPCB * cb)
{
	...
	SetConsoleTitle("TOPPERS/ASP SerialConsole");
...
static void ScreenBuffer_CreatePort(SIOPCB * cb)
{
	...
	// FatalAssertion(cb->Handle != INVALID_HANDLE_VALUE && cb->Handle != NULL,	"ScreenBuffer_CreatePort failed to allocate its own console.");
...
static void RawSerial_CreatePort(SIOPCB * cb)
{
	...
	// FatalAssertion(cb->Handle != INVALID_HANDLE_VALUE && cb->Handle != NULL, "RawSerial_CreatePort failed to open the port 'COM1'.");
...
sio_initialize(intptr_t exinf)
{
	...
	for (i = 0; i < TNUM_SIOP; i++) {
		...
		siopcb_table[i].Flag = SIO_TYP_TTY;
...
{
	// Windows 用に書き直す
	assert(p_siopcb != NULL);

	switch (SIO_TYP(p_siopcb->Flag))
	{
	case SIO_TYP_CONSOLE:
		// HALExecuteProcedure(SerialConsole_FinalRelease, p_siopcb);
		break;
	case SIO_TYP_TTY:
		WinConsole_ClosePort(p_siopcb);
		break;
	case SIO_TYP_SCRBUF:
		// HALExecuteProcedure(ScreenBuffer_ClosePort, p_siopcb);
		break;
	case SIO_TYP_RAWSIO:
		// HALExecuteProcedure(RawSerial_ClosePort, p_siopcb);
		break;
	}
...
sio_snd_chr(SIOPCB *p_siopcb, char c)
{
	...
	{
	case SIO_TYP_CONSOLE:
		result = SerialConsole_PushChar(p_siopcb, c);
		break;
	case SIO_TYP_TTY:
		result = WinConsole_PutChar(p_siopcb, c, TRUE);
		break;
	case SIO_TYP_SCRBUF:
		result = ScreenBuffer_PutChar(p_siopcb, c, TRUE);
		break;
	case SIO_TYP_RAWSIO:
		result = RawSerial_PutChar(p_siopcb, c, TRUE);
		break;
	}
...
sio_rcv_chr(SIOPCB *p_siopcb)
{
	...
	result = p_siopcb->ReceiveBuffer;
	p_siopcb->ReceiveBuffer = -1;
