...
#   の責任を負わない．
#  
#   $Id: componentobj.rb 2165 2014-12-31 14:00:28Z okuma-top $
#++
...
#     p "region: generate? #{@region.is_generate?}"
  ...
  def new_reverse_join( reverse_join )
    ...
    if ! @b_prototype then
      cdl_error( "S9999 '$1': reverse join is usable only in prototype cell", @name )
    end
...
#    rhs = Expression.new( [ :OP_DOT, [ :IDENTIFIER, Token.new( cell.get_name, nil, nil, nil ) ],
  ...
  def show_tree( indent )
    indent.times { print "  " }
    puts "Port: name:#{@name} port_type:#{@port_type} require:#{@b_require} inline:#{@b_inline} optional:#{@b_optional} ref_desc:#{@b_ref_desc} dynamic:#{@b_dynamic}"
    (indent+1).times { print "  " }
    if @signature then
...
# @cache_generating_region:: Region :  get_n_cells の結果をキャッシュするしているリージョン
 ...
 ### typedef
  ...
  def new_typedef( typedef )
    ...
    if dup != nil then
      if typedef.get_declarator.get_type.get_original_type.get_type_str == dup.get_declarator.get_type.get_original_type.get_type_str &&
          typedef.get_declarator.get_type.get_original_type.get_type_str_post == dup.get_declarator.get_type.get_original_type.get_type_str_post then
        # 同じものが typedef された
