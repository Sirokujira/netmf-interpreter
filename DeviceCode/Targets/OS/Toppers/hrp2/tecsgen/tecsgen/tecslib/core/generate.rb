...
#   の責任を負わない．
#  
#   $Id: generate.rb 2165 2014-12-31 14:00:28Z okuma-top $
#++
...
EOT
  ...
  #b_inline_only::  bool:   true ならば inline の entry port のみ、かつインアクティブなセルタイプを含める
  #  namespace "::" から呼出される
...
EOT
      ...
      f.print <<EOT
    #{necessity} #{@global_name}_CB	*p_cb;
    int		i;
    FOREACH_CELL(i,p_cb)
        SET_CB_INIB_POINTER(i,p_cb)
        INITIALIZE_CB(p_cb)
  ...
  def gen_template_cp_fun f
    ...
    @port.each { |p|
      ...
      if p.is_allocator_port? then
        ...
        f.print " * call port: #{p.get_name} signature: #{sig_name} #{context} #{optional}\n#{is_join}"
      else
