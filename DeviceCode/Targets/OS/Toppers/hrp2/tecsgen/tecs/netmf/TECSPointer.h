 ...
 *   の責任を負わない．
 *  
 *   $Id: TECSPointer.h 2032 2014-03-16 12:10:58Z okuma-top $ 
 */
...
#define	VALCHECK_INT(Type, TYPE, type )									\
	/*  check before assigning netmf_int val to intN_t */					\
	static inline void													\
	VALCHECK_ ## Type( netmf_state *netmf, netmf_int val )					\
	{																	\
		if( sizeof( netmf_int ) > sizeof( type ) ){						\
			if( val > TYPE ## _MAX || val < TYPE ## _MIN ) 				\
				netmf_raise(netmf, E_ARGUMENT_ERROR, "too large or too small for "  #type "_t"); \
		}																\
	}
#define	VALCHECK_MRB_INT(Type, TYPE, type )								\
	/*  check before assigning val (intN_t) to netmf_int */				\
	static inline void													\
	VALCHECK_MRB_ ## Type( netmf_state *netmf, type val )					\
	{																	\
		if( sizeof( type ) > sizeof( netmf_int ) ){						\
			if( val >= (((type)1) << (sizeof(netmf_int)*8-1))				\
				|| val < -(((type)1) << (sizeof(netmf_int)*8-1)) )		\
				/* '=' unecessary for negative value	*/				\
				/* ignore warning on int32_t */							\
				netmf_raise(netmf, E_ARGUMENT_ERROR, "too large or too small for netmf_int"); \
		}																\
	}
...
#define	VALCHECK_MRB_UINT(Type, TYPE, type )							\
	...
	{																	\
		if( sizeof( type ) > sizeof( netmf_int ) ){						\
			if( val >= (((type)1) << (sizeof(netmf_int)*8)))				\
				/* '=' unecessary for negative value	*/				\
				/* ignore warning on int32_t */							\
				netmf_raise(netmf, E_ARGUMENT_ERROR, "too large or too small for netmf_int"); \
		}																\
...
#define VALCHECK_Bool( netmf, val )
#define	VALCHECK_MRB_Bool( netmf, val )
...
#ifndef netmf_int_IS_int64
	 // ■ int 32 bit を仮定している. 64 bit であれば netmf_int_IS_64 を define する
...
static inline void
VALCHECK_Char( netmf_state *netmf, netmf_int  val )
{
	if( val > 0xff || val < 0 )	
		netmf_raise(netmf, E_ARGUMENT_ERROR, "too large or too small for char_t");
}
#define	VALCHECK_MRB_Char( netmf, val )

VALCHECK_INT( Short, SHRT, short )
#define	VALCHECK_MRB_Short( netmf, val )

VALCHECK_INT( Int, INT, int )
#define	VALCHECK_MRB_Int( netmf, val )

VALCHECK_INT( Long, LONG, long )
#define	VALCHECK_MRB_Long( netmf, val )

VALCHECK_INT( SChar, SCHAR, signed char )
#define	VALCHECK_MRB_SChar( netmf, val )

VALCHECK_UINT( UChar, UCHAR, unsigned char )
#define	VALCHECK_MRB_UChar( netmf, val )

VALCHECK_UINT( UInt, UINT, unsigned int )
#define	VALCHECK_MRB_UInt( netmf, val )

VALCHECK_UINT( UShort, USHRT, unsigned short )
#define	VALCHECK_MRB_UShort( netmf, val )

VALCHECK_UINT( ULong, ULONG, unsigned long )
#define	VALCHECK_MRB_ULong( netmf, val )

#else // TECS_NO_VAL_CHECK

#define VALCHECK_Char( netmf, val )
#define	VALCHECK_MRB_Char( netmf, val )
#define VALCHECK_Int( netmf, val )
#define	VALCHECK_MRB_Int( netmf, val )
#define	VALCHECK_Short( netmf, val )
#define	VALCHECK_MRB_Short( netmf, val )
#define	VALCHECK_Long( netmf, val )
#define	VALCHECK_MRB_Long( netmf, val )
#define	VALCHECK_SChar( netmf, val )
#define	VALCHECK_MRB_SChar( netmf, val )
#define	VALCHECK_UChar( netmf, val )
#define	VALCHECK_MRB_UChar( netmf, val )
#define	VALCHECK_UInt( netmf, val )
#define	VALCHECK_MRB_UInt( netmf, val )
#define	VALCHECK_UShort( netmf, val )
#define	VALCHECK_MRB_UShort( netmf, val )
#define	VALCHECK_ULong( netmf, val )
#define	VALCHECK_MRB_ULong( netmf, val )

#endif // TECS_NO_VAL_CHECK

#define CHECK_AND_GET_POINTER( Type, type )								\
	type *CheckAndGet ## Type ## Pointer( netmf_state *netmf, netmf_value value, int32_t len ) \
	{																	\
		if( netmf_type(value) != MRB_TT_DATA								\
			|| DATA_TYPE( value ) == 0									\
			|| strcmp( DATA_TYPE( value )->struct_name,  "TECS::" #Type "Pointer" ) ) \
			netmf_raise(netmf, E_TYPE_ERROR, "not PointerType or unexpected PointerType"); \
		if( len != -1 && ((struct Type ## PointerBody *)DATA_PTR( value ))->length < len ) \
			netmf_raise(netmf, E_ARGUMENT_ERROR, "size_is too short");		\
		return ((struct Type##PointerBody*)(DATA_PTR(value)))->buf;		\
	}																	\
																		\
	type *CheckAndGet ## Type ## PointerNullable( netmf_state *netmf, netmf_value value, int32_t len ) \
	{																	\
		if( netmf_nil_p(value) )											\
			return NULL;												\
		return CheckAndGet ## Type ## Pointer( netmf, value, len );		\
	}
...
#define CHECK_AND_GET_CHAR_POINTER( Type, type )								\
	type *CheckAndGet ## Type ## Pointer( netmf_state *netmf, netmf_value value, int32_t len ) \
	{																	\
		if( netmf_type(value) != MRB_TT_STRING )							\
			netmf_raise(netmf, E_TYPE_ERROR, "not String Type");			\
...
#define CHECK_AND_GET_CHAR_POINTER_MOD( Type, type )					\
	type *CheckAndGet ## Type ## PointerMod( netmf_state *netmf, netmf_value value, int32_t len ) \
	{																	\
		char *ptr;														\
		if( netmf_type(value) != MRB_TT_STRING )							\
			netmf_raise(netmf, E_TYPE_ERROR, "not String Type");			\
		str_modify(netmf,RSTRING(value));									\
		if( len == -1 || RSTRING_LEN(value) < len )						\
			netmf_raise(netmf, E_ARGUMENT_ERROR, "size_is too short");		\
		...
		return (type *)ptr;												\
																		...
																		\
	type *CheckAndGet ## Type ## PointerModNullable( netmf_state *netmf, netmf_value value, int32_t len ) \
	{																	\
		if( netmf_nil_p(value) )											\
			return NULL;												\
		return CheckAndGet ## Type ## Pointer( netmf, value, len );		\
...
#define GET_SET_BOOL( Type, TYPE )										\
	static netmf_value													\
	Type ## Pointer_aget(netmf_state *netmf, netmf_value self)				\
	{																	\
		struct Type ## PointerBody *pointerBody = DATA_PTR( self );		\
		netmf_int	subscript;												\
		TYPE	val;													\
																		\
		netmf_get_args(netmf, "i", &subscript );							\
		if( subscript < 0 || subscript >= pointerBody->length )			\
			netmf_raise(netmf, E_INDEX_ERROR, "index is out of array (get)"); \
		val = pointerBody->buf[ subscript ];							\
		VALCHECK_MRB_ ## Type( netmf, val );								\
		return val ? netmf_true_value() : netmf_false_value();				\
	}																	\
																		\
	static netmf_value													\
	Type ## Pointer_aset(netmf_state *netmf, netmf_value self)				\
	{																	\
		struct Type ## PointerBody *pointerBody = DATA_PTR( self ); 	\
		netmf_int	subscript;												\
		netmf_value	val;													\
																		\
		netmf_get_args(netmf, "io", &subscript, &val );						\
																		\
		if( subscript < 0 || subscript >= pointerBody->length )			\
			netmf_raise(netmf, E_INDEX_ERROR, "index is out of array (set)"); \
		pointerBody->buf[ subscript ] = netmf_test( val );				\
																		\
		return self;													\
	}																	\
																		\
	static netmf_value													\
	Type ## Pointer_get_val(netmf_state *netmf, netmf_value self)				\
	{																	\
		struct Type ## PointerBody *pointerBody = DATA_PTR( self );		\
		TYPE	val;													\
																		\
		...
		return val ? netmf_true_value() : netmf_false_value();				\
																		...
																		\
	static netmf_value													\
	Type ## Pointer_set_val(netmf_state *netmf, netmf_value self)				\
	{																	\
		struct Type ## PointerBody *pointerBody = DATA_PTR( self ); 	\
		netmf_value	val;												\
																		\
		netmf_get_args(netmf, "o", &val );									\
																		\
		*pointerBody->buf = netmf_test( val );							\
		return self;													\
	}
...
#define GET_SET_INT( Type, TYPE )										\
	...
	{																	\
		...
		netmf_int	val;													\
																		\
		netmf_get_args(netmf, "ii", &subscript, &val );						\
																		\
		if( subscript < 0 || subscript >= pointerBody->length )			\
			netmf_raise(netmf, E_INDEX_ERROR, "index is out of array (set)"); \
...
#define GET_SET_FLOAT( Type, TYPE )										\
	...
	{																	\
		...
		if( subscript < 0 || subscript >= pointerBody->length )			\
			netmf_raise(netmf, E_INDEX_ERROR, "index is out of array (get)"); \
		return netmf_float_value( netmf, pointerBody->buf[ subscript ] );   \
	}																	\
																		\
	static netmf_value													\
	Type ## Pointer_aset(netmf_state *netmf, netmf_value self)				\
	{																	\
		...
		netmf_float	val;												\
																		\
		netmf_get_args(netmf, "if", &subscript, &val );						\
																		\
		if( subscript < 0 || subscript >= pointerBody->length )			\
			netmf_raise(netmf, E_INDEX_ERROR, "index is out of array (set)"); \
		pointerBody->buf[ subscript ] = val;							\
	...
	{																	\
		struct Type ## PointerBody *pointerBody = DATA_PTR( self );		\
																		\
		return netmf_float_value( netmf, *pointerBody->buf );               \
	}																	\
																		\
	static netmf_value													\
	Type ## Pointer_set_val(netmf_state *netmf, netmf_value self)				\
...
#define POINTER_CLASS( Type, TYPE )										\
																		\
	static void															\
	Type ## PointerBody_free( netmf_state *netmf, void *p )					\
	{																	\
		(void)netmf_free( netmf, p );										\
	}																	\
																		\
	struct netmf_data_type Type ## PointerBody_netmf_data_type =			\
		{																\
			"TECS::" #Type "Pointer",									\
			Type ## PointerBody_free									\
																		...
																		\
	static netmf_value													\
	Type ## Pointer_initialize( netmf_state *netmf, netmf_value self)			\
	{																	\
		netmf_int	length;													\
		struct Type ## PointerBody *pointerBody;						\
																		\
		netmf_get_args(netmf, "i", &length );								\
		if( length < 0 )												\
			netmf_raise(netmf, E_INDEX_ERROR, "negative length");			\
		DATA_TYPE( self ) = &Type ## PointerBody_netmf_data_type;			\
		pointerBody = (struct Type ## PointerBody *)netmf_malloc(netmf, sizeof(struct Type ## PointerBody) + length * sizeof( TYPE ) ); \
		...
		return self;													\
																		...
																		\
	static netmf_value													\
	Type ## Pointer_size(netmf_state *netmf, netmf_value self)				\
	{																	\
		struct Type ## PointerBody *pointerBody = DATA_PTR( self );		\
																		\
		return netmf_fixnum_value( pointerBody->length );					\
	}																	\
																		\
	struct RClass *														\
	tecs_init_## Type ## Pointer(netmf_state *netmf, struct RClass *TECS)	\
	{																	\
		struct RClass *a;												\
																		\
		a = netmf_define_class_under(netmf, TECS, #Type "Pointer", netmf->object_class); \
		MRB_SET_INSTANCE_TT(a, MRB_TT_DATA);							\
																		\
		netmf_define_method(netmf, a, "initialize",      Type ## Pointer_initialize,   ARGS_REQ(1)); \
		netmf_define_method(netmf, a, "[]",              Type ## Pointer_aget,         ARGS_REQ(1)); \
		netmf_define_method(netmf, a, "value",           Type ## Pointer_get_val,      ARGS_NONE()); \
		netmf_define_method(netmf, a, "[]=",             Type ## Pointer_aset,         ARGS_REQ(2)); \
		netmf_define_method(netmf, a, "value=",          Type ## Pointer_set_val,      ARGS_REQ(1)); \
		netmf_define_method(netmf, a, "size",            Type ## Pointer_size,         ARGS_NONE()); \
		netmf_define_method(netmf, a, "length",          Type ## Pointer_size,         ARGS_NONE()); \
		return	a;														\
	}

static inline netmf_value
CharPointer_to_s( netmf_state *netmf, netmf_value self )
{
	struct CharPointerBody *pointerBody = DATA_PTR( self );
	return netmf_str_new( netmf, pointerBody->buf, strnlen( pointerBody->buf, pointerBody->length ) );
}

static inline netmf_value
CharPointer_from_s( netmf_state *netmf, netmf_value self )
{
	netmf_value str;
	struct CharPointerBody *pointerBody = DATA_PTR( self );

	netmf_get_args(netmf, "o", &str );
	if( netmf_type( str ) != MRB_TT_STRING ){
		netmf_raise(netmf, E_ARGUMENT_ERROR, "Not String");
	}
...
void	init_TECSPointer( netmf_state *netmf, struct RClass *TECS );
