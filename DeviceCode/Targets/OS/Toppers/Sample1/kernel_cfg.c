...
/*
 *  Variable-sized Memorypool Functions
 */

const ID _kernel_tmax_mplid = (TMIN_MPLID + TNUM_MPLID - 1);

TOPPERS_EMPTY_LABEL(const MPLINIB, _kernel_mplinib_table);
TOPPERS_EMPTY_LABEL(MPLCB, _kernel_mplcb_table);
...
const CYCINIB _kernel_cycinib_table[TNUM_CYCID] = {
	{ (TA_NULL), (intptr_t)(0), (CYCHDR)(cyclic_handler), (2000), (0) }
};
