...
{
    return MBED_GR_PEACH_TouchPanel_Driver::Initialize(touchIsrProc);
}

BOOL TOUCH_PANEL_Driver_Disable()
{
    return MBED_GR_PEACH_TouchPanel_Driver::Uninitialize();
}

void TOUCH_PANEL_Driver_GetPoint(TOUCH_PANEL_SAMPLE_FLAGS* pTipState, int* pSource, int* pUnCalX, int* pUnCalY)
{
    MBED_GR_PEACH_TouchPanel_Driver::GetPoint(pTipState, pSource, pUnCalY, pUnCalX);
}

HRESULT TOUCH_PANEL_Driver_GetDeviceCaps(unsigned int iIndex, void* lpOutput)
{
    return MBED_GR_PEACH_TouchPanel_Driver::GetDeviceCaps(iIndex, lpOutput);
}
